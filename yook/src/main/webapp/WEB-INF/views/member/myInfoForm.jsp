					...
					<div data-v-4e5d839d="" class="mypage-userinfo__data-top"
						...
						<div style="margin-left: 450px;">
							<p data-v-4e5d839d="" class="mypage-userinfo__name"
								style="display: inline-block; font-size: 24px; font: bold;">Hello,
								${session_MEM_ID}님 &nbsp;&nbsp;</p>

							<a href="${contextPath}/yook/logout.do"
								style="color: gray; font-size: 14px;">[로그아웃]</a>

						</div>
						<ul style="margin-right: 100px;">
							<li align="left" data-v-4e5d839d=""
								class="mypage-userinfo__data-list"
								style="display: inline-block;">
								<div data-v-4e5d839d="" class="mypage-userinfo__data-centered"
							...
							<div data-v-4e5d839d="" class="mypage-userinfo__data-centered"
								...
								<p data-v-4e5d839d="">
									<c:choose>
										<c:when test="${map.MEM_GRADE eq 1}">[웰컴]</c:when>
							...
							<li data-v-4e5d839d="" class="mypage-userinfo__data-list"
								style="display: inline-block; outline: 1px; margin-left: 50px;">
							<div data-v-4e5d839d="" class="mypage-userinfo__data-centered"
								style="float: left; width: 100%;">
							...
							<div data-v-4e5d839d="" class="mypage-userinfo__data-centered"
								...
								<p data-v-4e5d839d="">
								<fmt:formatDate pattern="yyyy/MM/dd" value="${map.MEM_JOINDATE}" />
							</div>
							</li>
						</ul>
					</div>
			...
			<div class="container">
				...
				<ul class="nav  justify-content-center nav-tabs nav-pills"
					...
					<li class="nav-item"><a class="nav-link active"
						data-toggle="tab" href="#home">주문내역</a></li>

					<li class="nav-item"><a class="nav-link" data-toggle="tab"
						href="#menu2">회원정보수정</a></li>
				</ul>
				...
				<div class="tab-content">
					<div id="home" class="container  tab-pane active">
						...
						<main role="main text-center">
							...
							<div class="container" style="width: 100%;">
								<table class="table"
									style="cellpadding: 10; cellspacing: 10; border: 1px solid #bdbdbd"
									align="center">
												...
												<c:when test="${fn:length(list) > 0 }">
													<c:forEach items="${list}" var="row">
														<tr style="text-align: center;">
															...
															<td style="color: gray;"><font
																style="font-size: 10pt;">${row.ORDER_NUM}</font></td>
															<td><img
																src="/yook/img/goods_upload/${row.GOODS_IMAGE}"
																width="100px" height="100px"></td>

															<td><strong><a href="#this" name="detail"
																	style="text-decoration: none; color: black;">${row.GOODS_NAME}<br />${row.ORDER_DETAIL_WEIGHT }
																		<input type="hidden" id="MEM_NUM" name="MEM_NUM"
																		value="${session_MEMBER.MEM_NUM}"> <input
																		type="hidden" id="ORDER_NUM" name="ORDER_NUM"
																		value="${row.ORDER_NUM}"> <input type="hidden"
																		id="GOODS_NAME" name="GOODS_NAME"
																		value="${row.GOODS_NAME}"> <input
																		type="hidden" id="GOODS_NUM" name="GOODS_NUM"
																		value="${row.GOODS_NUM}"> <input type="hidden"
																		id="ORDER_DETAIL_WEIGHT" name="ORDER_DETAIL_WEIGHT"
																		value="${row.ORDER_DETAIL_WEIGHT}">
																</a></strong></td>


															<td>${row.ORDER_DETAIL_AMOUNT}개</td>
															<td>${row.GOODS_PRICE * row.ORDER_DETAIL_AMOUNT}원</td>
															<td><c:if test="${row.AORDER_STATE == '0'}">
												               입금전
												               </c:if> <c:if test="${row.AORDER_STATE == '1'}">
												               배송준비중
												               </c:if> <c:if test="${row.AORDER_STATE == '2'}">
												               배송중
												               </c:if> <c:if test="${row.AORDER_STATE == '3'}">
												               배송완료
												               </c:if> <c:if test="${row.AORDER_STATE == '4'}">
												               취소요청
												               </c:if> <c:if test="${row.AORDER_STATE == '5'}">
												               취소처리
												               </c:if> <c:if test="${row.AORDER_STATE == '6'}">
												               교환요청
												               </c:if> <c:if test="${row.AORDER_STATE == '7'}">
												               교환중
												               </c:if> <c:if test="${row.AORDER_STATE == '8'}">
												               교환처리
												               </c:if> <c:if test="${row.AORDER_STATE == '9'}">
												               반품요청
												               </c:if> <c:if test="${row.AORDER_STATE == '10'}">
												               반품처리
												               </c:if></td>
															<td><a href="#this" name="shit"> <input
																	type="button" class="btn btn-outline-primary"
																	value="취소/교환/반품" /> <input type="hidden" id="MEM_NUM"
																	name="MEM_NUM" value="${session_MEMBER.MEM_NUM}">
					...
					<div id="menu2" class="container tab-pane">
						...
						<form id="frm" name="frm" class="form-inline" method="post">
							<table class="joinFormView"
								style="border: 0; cellpadding: 1; cellspacing: 1;"
								align="center">
								<tr>
									<td style="width: 150px; padding-right: 15px;" align="right">
										사용자 ID</td>
									<td colspan="2"><input type="text" id="MEM_ID"
										name="MEM_ID" size="15" maxlength="12" class="form-control"
										placeholder="아이디" value="${map.MEM_ID}" readonly
										style="width: 100%;"></td>
								</tr>
								...
								<tr>
									<td style="width: 150px; padding-right: 15px;" align="right">
										비밀번호</td>
									<td colspan="2"><input type="password" id="MEM_PW"
										name="MEM_PW" size="15" maxlength="12" class="form-control"
										placeholder="비밀번호" style="width: 100%;"></td>
								</tr>

								<tr>
									<td style="width: 150px; padding-right: 15px;" align="right">
										비밀번호 확인</td>
									<td colspan="2"><input type="password" id="MEM_PW2"
										name="MEM_PW2" size="15" maxlength="12" class="form-control"
										placeholder="비밀번호 확인" style="width: 100%;"></td>
								</tr>
								...
								<tr>
									...
									<td colspan="2"><input type="text" id="MEM_NAME"
										name="MEM_NAME" size="15" maxlength="12" class="form-control"
										placeholder="이름" value="${map.MEM_NAME}" readonly 
										style="width: 100%;"></td>
								</tr>
								<tr>
									<td style="width: 150px; padding-right: 15px;" align="right">이메일</td>
									<td colspan="2"><input type="text" id="MEM_EMAIL"
										name="MEM_EMAIL" size="40" maxlength="30" class="form-control"
										placeholder="이메일" value="${map.MEM_EMAIL}"
										style="width: 100%;"></td>
								</tr>
								...
								<tr>
									<td style="width: 150px; padding-right: 15px;" align="right">
										연락처</td>
									<td colspan="2"><input type="text" id="MEM_PHONE"
										name="MEM_PHONE" size="40" maxlength="12"
										placeholder="연락처(- 없이 숫자만 입력)" value="${map.MEM_PHONE}"
										oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"
										class="form-control" placeholder="PHONE" style="width: 100%;">
									</td>
								</tr>
								...
								<tr>
									<td style="width: 150px; padding-right: 15px;" align="right"
										rowspan="2">주 소</td>
									<td><input type="text" id="MEM_ZIPCODE" name="MEM_ZIPCODE"
										placeholder="우편번호" size="15" class="form-control"
										value="${map.MEM_ZIPCODE}" readonly style="width: 100%;">
									</td>
									<td><input type="button" onclick="DaumAddressAPI()"
										class="btn btn-outline-primary mb-2" value="우편번호 찾기"
										style="width: 100%;"><br></td>
								</tr>
								<tr>
									<td colspan="2"><input type="text" name="MEM_ADDRESS1"
										id="MEM_ADDRESS1" placeholder="주소" value="${map.MEM_ADDRESS1}"
										size="40" class="form-control" style="width: 100%;"><br>
										<input type="text" name="MEM_ADDRESS2" id="MEM_ADDRESS2"
										placeholder="상세주소" value="${map.MEM_ADDRESS2}" size="40"
								...
								<tr>
									<td colspan="3" align="center"><br> <a href="#this"
										class="btn" id="modify"><input type="button"
											class="btn btn-primary mb-2" value="수정하기"
											style="margin-left: 90px;" /></a> <input type="button"
										value="취 소" class="btn btn-outline-primary mb-2"
										onclick="javascript:window.location='/muscle/member/openMyInfoForm.do'" />&nbsp;&nbsp;&nbsp;
										<a href="#this" class="btn" id="delete"><input
											type="button" value="회원탈퇴"
											class="btn btn-outline-primary mb-2" /></a></td>
								</tr>
					...
					<script type="text/javascript">
						...
						function fn_modifySc() {
							...
							comSubmit
									.setUrl("<c:url value='/updateMyInfo.do' />");
						...
						function fn_delete() {
							...
							if (del) {
								...
								comSubmit
										.setUrl("<c:url value='/deleteMyInfo.do' />");
								comSubmit.submit();
							} else {
								history.go(-1);
							}
					...
					<script type="text/javascript">
						...
						function fn_shit(obj) {
							...
							comSubmit
									.setUrl("<c:url value='/openMyChangeForm.do' />");
							comSubmit.addParam("MEM_NUM", obj.parent().find(
									"#MEM_NUM").val());
							comSubmit.addParam("ORDER_NUM", obj.parent().find(
									"#ORDER_NUM").val());
							comSubmit.addParam("ORDER_DETAIL_WEIGHT", obj
									.parent().find("#ORDER_DETAIL_WEIGHT")
									.val());
							comSubmit.addParam("GOODS_NUM", obj.parent().find(
									"#GOODS_NUM").val());
							comSubmit.submit();
						}
						...
						function fn_det(obj) {
							var comSubmit = new ComSubmit("frm1");
							comSubmit
									.setUrl("<c:url value='/openMyOrderDetail.do' />");
							comSubmit.addParam("MEM_NUM", obj.parent().find(
									"#MEM_NUM").val());
							comSubmit.addParam("ORDER_NUM", obj.parent().find(
									"#ORDER_NUM").val());
							comSubmit.addParam("GOODS_NAME", obj.parent().find(
									"#GOODS_NAME").val());
